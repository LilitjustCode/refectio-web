...
export const Auth_reducer = (state = store, action) => {
  ...
  switch (action.type) {
    case "login":
      if (action.payload.status) {
        ...
        if (action.payload?.message?.message?.includes("user does not exist")) {
          ...
          action.payload?.message?.message?.includes("wrong password")
        ) {
          temp.loginError = "Неправильный номер телефона или пароль";
    ...
    case "getAllCountries":
      if (action.payload.status) {
        temp.countries = action.payload.new_order_data;
      }
      break;
    case "sendCall":
      if (
        action.payload.message === "Green Error Pleace send code 10 minute ago"
      ) {
    ...
    case "verifyCode":
      if (action.payload.status) {
        ...
        action.payload?.message[0]?.includes("wrong verification code")
      ) {
