...
export const SecondRegister = () => {
  ...
  const [show_password_confirmation, setShow_password_confirmation] =
    useState(false);
  ...
  function handleImageChange(e) {
    if (e.target.files.length) {
      ...
      document.getElementById(
        "avatar"
      ).innerHTML = `<img alt='' src=${URL.createObjectURL(
        e.target.files[0]
      )} />`;
    }
  }
  ...
  function register() {
    ...
    if (!details?.name?.length) {
      ...
      setErrors({
        ...
        whatsapp: "",
        country: "",
        password: "Пароль должен содержать не менее 6-ти символов",
      });
      ...
      setErrors({
        ...
        country: "",
        password: "",
        password_confirmation: "Пароль должен содержать не менее 6-ти символов",
      });
    ...
    } else if (details?.admin_key.length < 10) {
      setErrors({
        ...errors,
        name: "",
        phone: "",
      ...
      formdata.append("admin_key", details?.admin_key);
      ...
      fetch(`${process.env.REACT_APP_HOSTNAME}/RegisterManufacturerUser`, {
        ...
        .then((result) => {
          ...
          if (result.status) {
            ...
            result?.message?.includes("user@ chi ancel hamari verifykacia")
          ) {
            // setOpenCode(true);
            window.location = "/auth/login";
            setToken(result?.token);
          } else if (
            result?.data?.company_name[0].includes(
  ...
  return (
    <div className="loginPage">
      {/* {openCode && (
        <ConfirmCode open={openCode} setOpen={setOpenCode} token={token} />
      )} */}
      <div className="loginBlock" style={{ overflow: "auto" }}>
        ...
        <div className="loginInputs">
          <label>
            Номер Whatsapp-для запроса стоимости <Asterisk />
          </label>
          <ReactInputMask
            mask="+7 (999) 999-99-99"
            maskChar="_"
            ...
            onChange={(e) =>
              setDetails({ ...details, whatsapp: e.target.value })
            }
            style={errors?.whatsapp ? { border: "1px solid red" } : {}}
          />
          {errors?.whatsapp?.length > 0 && (
            <span className="errorMessage">{errors?.whatsapp}</span>
          )}
        </div>
        <div className="eachProfileField loginInputs">
          <label>
            Страна производства <Asterisk />
          </label>
          <select
        ...
        <div className="loginInputs">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (newValue.startsWith("t.me/")) {
                setDetails({ ...details, telegram: e.target.value });
              } else {
                setDetails({ ...details, telegram: `t.me/${newValue}` });
              }
            }}
          />
        </div>
        <div className="eachProfileField loginInputs">
          ...
          <select
            value={details?.showroom}
            onChange={(e) =>
              setDetails({ ...details, showroom: e.target.value })
            }
          >
        ...
        <div className="eachProfileField loginInputs">
          ...
          <select
            ...
            onChange={(e) =>
              setDetails({ ...details, designer: e.target.value })
            }
          >
            <option value={"Да"}>Да</option>
            <option value={"Нет"}>Нет</option>
        ...
        <div className="eachProfileField loginInputs">
          ...
          <select
            ...
            onChange={(e) => setDetails({ ...details, models: e.target.value })}
          >
            <option value={"Нет"}>Нет</option>
            <option value={"Да"}>Да</option>
          </select>
        ...
        <div className="loginInputs">
          ...
          <div className="loginPaswordInput">
            <input
              ...
              style={
                errors.password || loginError ? { border: "1px solid red" } : {}
              }
            />
        ...
        <div className="loginInputs">
          ...
          <div className="loginPaswordInput">
            <input
              ...
              style={
                errors.password_confirmation || loginError
            ...
            <div
              onClick={() =>
                setShow_password_confirmation(!show_password_confirmation)
              }
            >
        ...
        <div className="loginInputs">
          <label>
            Загрузитье аватар/логотип <Asterisk />
          </label>
          <div id="avatar" />
          <button>
            Загрузить
          ...
          {errors?.logo?.length > 0 && (
            <span className="errorMessage">{errors?.logo}</span>
          )}
        </div>
        <div className="loginInputs">
          ...
          <input
            ...
            max={20}
            required
            value={details?.admin_key}
            onChange={(e) =>
              e.target.value.length < 20 &&
              setDetails({ ...details, admin_key: e.target.value })
            }
          />
        </div>
        <div className="loginInputs">
          <div className="checkboxLine">
            <div
              ...
              onClick={() =>
                setDetails({ ...details, checkbox: !details?.checkbox })
              }
            >
              {details?.checkbox ? <RegisterCheckbox /> : ""}
            </div>
            <p>
              Согласен с правилами <span>приложения</span> и{" "}
              <span>политикой конфиденциальности</span>
            </p>
          </div>
          {errors?.checkbox?.length > 0 && (
            <span className="errorMessage">{errors?.checkbox}</span>
          )}
        </div>
