      ...
      <div className="pop" style={{ width: "460px" }}>
        ...
        <div className="loginInputs">
          ...
          <input
            ...
            style={
              codeError.length > 0 || codeErrorBack?.length > 0
                ? { border: "1px solid red" }
                : {}
            }
            onKeyDown={(e) => e.key === "Enter" && confirm()}
          />
          {!codeErrorBack && codeError && (
            <span className="errorMessage">{codeError}</span>
          )}
          {codeErrorBack && (
            <span className="errorMessage">{codeErrorBack}</span>
          )}
          <div className="timer">
            <span
              onClick={(e) => {
            ...
            {counter > 0 && (
              <p>Вы можете отправить код повторно через 00 : {counter}</p>
            )}
          </div>
        </div>
