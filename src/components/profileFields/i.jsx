...
export const ProfileFields = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      user?.city_of_sales_manufacturer?.forEach((elm) => {
        city.push({
          label: elm?.city_name,
          value: `${elm?.city_id}^${elm?.city_name}`,
        });
      });
      setSelected(city);
      setUserDetails({
  ...
  function uploadSingleFile(e) {
    if (e.target.files.length) {
      ...
      fetch(`${process.env.REACT_APP_HOSTNAME}/updateLogoProizvoditel`, {
        ...
        .then(
          (result) =>
            result.status && setFile(URL.createObjectURL(e.target.files[0]))
        )
  ...
  return (
    <>
      ...
      {openPhone && <EditPhone open={openPhone} setOpen={setOpenPhone} />}
      {Object.keys(user).length ? (
        <div className="myProfileBlock">
          <div className="profileMiddleBlocks">
            ...
            <div className="eachProfileField">
              ...
              <div className="profileFieldName">
                ...
                <div
                  className="cursor"
                  onClick={() => setEdit({ ...edit, country: true })}
                >
                  <EditIcon />
                </div>
              </div>
            ...
            {edit?.country && (
              <div>
                <button
                  ...
                  onClick={() =>
                    userDetails?.country.length > 0
                      ? dispatch(UpdateCountry(userDetails?.country))
                      : setEdit({ ...edit, country: false })
                  }
                >
                  Обновить
                </button>
            ...
            <div className="eachProfileField">
              ...
              <div className="profileFieldName">
                ...
                <div
                  className="cursor"
                  onClick={() => setEdit({ ...edit, code: true })}
                >
                  <EditIcon />
                </div>
              </div>
            ...
            {edit?.code && (
              <div>
                <button
                  className="profileEditButton"
                  onClick={() =>
                    userDetails?.code?.length > 0
                      ? dispatch(UpdateCode(userDetails?.code))
                      : setEdit({ ...edit, code: false })
                  }
                >
                  Обновить
                </button>
            ...
            <div className="eachProfileField">
              ...
              {edit?.description ? (
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: userDetails?.description }}
                />
              )}
            </div>
            {edit?.description && (
              <div>
                <button
                  className="profileEditButton"
                  onClick={() =>
                    dispatch(UpdateAbout(userDetails?.description))
                  }
                >
                  Обновить
                </button>
          ...
          <div className="profileMiddleBlocks">
            <div id="eachItem" className="eachProfileField">
              {" "}
              {/* Название */}
              <div className="profileFieldName">
                <span>Название</span>
              ...
              <input
                ...
                style={
                  nameError.length || !userDetails?.name?.length
                    ? { border: "1px solid red" }
                    : edit.name
                    ? { border: "3px solid #bebebe" }
                    : { border: "1px solid #bebebe" }
              ...
              <div>
                <button
                  ...
                  onClick={() =>
                    ...
                    dispatch(UpdateName(userDetails?.name))
                  }
                >
                  Обновить
                </button>
            ...
            <div className="eachProfileField">
              ...
              <div className="profileFieldName">
                ...
                <div
                  className="cursor"
                  onClick={() => setEdit({ ...edit, telegram: true })}
                >
                  <EditIcon />
                </div>
              </div>
            ...
            {edit?.telegram && (
              <div>
                <button
                  className="profileEditButton"
                  onClick={() =>
                    userDetails?.telegram?.length > 0
                      ? dispatch(UpdateTelegram(userDetails?.telegram))
                      : setEdit({ ...edit, telegram: false })
                  }
                >
                  Обновить
                </button>
            ...
            <div className="eachProfileField">
              ...
              <div className="profileFieldName">
                ...
                <div
                  className="cursor"
                  onClick={() => setEdit({ ...edit, site: true })}
                >
                  <EditIcon />
                </div>
              </div>
              <input
                ...
                onChange={(e) =>
                  setUserDetails({ ...userDetails, site: e.target.value })
                }
              />
            </div>
            {edit?.site && (
              <div>
                <button
                  className="profileEditButton"
                  onClick={() =>
                    userDetails?.site?.length > 0
                      ? dispatch(UpdateSite(userDetails?.site))
                      : setEdit({ ...edit, site: false })
                  }
                >
                  Обновить
                </button>
